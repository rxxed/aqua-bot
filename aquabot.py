...
def handle_user_messages(updates):
    for update in updates["result"]:
        try:
            ...
            if is_under_command:
                if command_name == 'setInterval':
                    if text.isdigit():
                        interval_seconds = text
                    else:
                        send_message("Not a number")
...
def set_interval():
    ...
    message = f"So you want an interval of {interval} seconds"
    send_message(message, chatid)
...
def main():
    ...
    while True:
        ...
        if len(updates["result"]) > 0:
            ...
            is_under_command = False
            handle_user_messages(updates, is_under_command)
